...
def parse_arguments(argv):
    ...
    parser.add_argument('--data_dir',
                        type=str,
                        default='./images/train/xy12_images/',
...
def import_train_data(path, dim):
    ...
    # Save the classes
    np.save('./models/classes.npy', np.unique(y))
...
def build_sampler(x_train, y_train, argv=None):
    ...
    def augmenter(
        x: tfsim.types.FloatTensor, y: tfsim.types.IntTensor, examples_per_class: int, is_warmup: bool, stddev=0.1
...
def train(argv=None):
    ...
    model.reset_index()
    model.index(x_index, y_index, data=x_index, verbose=0)
